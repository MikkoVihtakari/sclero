...
#' dat <- order.ijdata(shellspots, gbs = c(1:6,13,14,15,7,11,12,8,9,10)) # Changes order of growth bands
#' order.ijdata(dat, print.order = TRUE)
#' 
#' ## Subset the first sample spot sequence
#' dat2 <- order.ijdata(shellspots, spots = 1)
#' ## Warning message:
#' ## In order.ijdata(dat, spots = 1) :
#' ## Length of spots does not correspond the number of columns. Data removed.
