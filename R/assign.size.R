...
#' @param file optional. ImageJ .zip file containing the spot size information. If \code{NULL}, the file is assumed to be the same than from where 'rawDist' \link[=read.ijdata]{data was read from}. The file must be located in the working directory.
...
assign.size <- function(rawDist, file = NULL, names = "generate.invalid", types = c("oval", "freehand", "rect")){

Z <- rawDist

## 1. Read in the .zip file and convert to spatstat
  if(is.null(file)){
    file <- paste0(rawDist$sample.name, ".zip")}
  if(!file %in% dir()) stop(paste0("Cannot find ", file, ". Check getwd() or define file"))
  X <-  read.ijzip(file, names = TRUE)
