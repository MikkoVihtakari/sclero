...
#' @param alignment Should the sample spots be aligned according to the shortest distance to surrounding growth lines (default, \code{closest}) or according to the distance to the crossing points of the sample sequence traverse and growth lines (\code{traverse}, see 'Details' for further information.)  
#' @param sample.name An optional parameter over-riding the sample name. If left empty the sample name will be passed from the previous steps (\code{\link{read.ijdata}}, \code{\link{convert.ijdata}})
...
#' @details daa
#' @return Returns a list of class \code{spotDist}, which contains \link[spatstat]{spatstat} point patterns. The returned \code{rawDist} can be plotted using the generic plotting command.
...
spot.dist <- function(rawDist, alignment = "closest", sample.name = NULL, run.ae = TRUE, use.centroids = TRUE){
