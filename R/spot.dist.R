...
spot.dist <- function(rawDist, coord.type = "scaled", sample.name = NULL, run.ae = TRUE, use.centroids = TRUE){
  ...
  spot.dist.calculation <- function(spots, gbs, lines, dist.main, l.1st) {
    ...
    for(l in 1:length(spots)){
      ...
      for(j in 1:nlines){
        temp <- spatstat::nncross(spots[[l]], gbs[gbs$marks == lines[j]])
    ...
    ## Part 2. Calculate whether spots are occuring after or before (from l.1st) the closest subannual line
    ...
    tmp.l <- lapply(seq_along(spots), function(k) {
      tmp <- c()
      for(j in 1:spatstat::npoints(spots[[k]])) {
        tmp <- spatstat::superimpose(tmp, spatstat::project2segment(spots[[k]][j], gbs[gbs$marks == d[[k]]$close1[j]])$Xproj)
      }
      spatstat::nncross(tmp, l.1st)[,1]
    })
  ...
  if(all(names(rawDist) %in% c("scaled", "original"))) {
