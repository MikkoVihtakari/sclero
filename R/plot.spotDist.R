...
#' @param growth.axis character defining the color of the growth axis. Set to \code{NULL} to remove the growth axis. 
...
# Debug parameters
# x = aligned; sample.name = "keep"; spot.type = "id"; spot.size = 2; spot.color = NULL; main.type = "all"; growth.axis = NULL; color.palette = colorRampPalette(c("blue", "cyan", "yellow", "red"), bias=1)(100); highlight.gbs = NULL; highlight.col = "red"; crossing.points = FALSE

plot.spotDist <- function(x, ..., sample.name = "keep", spot.type = "id", spot.size = 2, spot.color = NULL, main.type = "all", growth.axis = NULL, color.palette = colorRampPalette(c("blue", "cyan", "yellow", "red"), bias=1)(100), highlight.gbs = NULL, highlight.col = "red", crossing.points = FALSE){
  ...
  text(spatstat::coords(x$gb.projections)[,1], spatstat::coords(x$gb.projections)[,2], x$gb.projections$marks, col = "blue", cex = 0.6, srt = 90, font = 2)
  ...
  start <- x$gb.start
  x.new <- lapply(print, function(i) {
    (i[,2] / c(spatstat::crossdist(start, end))) * coords(end)$x + (1 - (i[,2] / c(spatstat::crossdist(start, end)))) * spatstat::coords(start)$x
  })
  y.new <- lapply(print, function(i) {
    (i[,2] / c(spatstat::crossdist(start, end))) * coords(end)$y + (1-(i[,2] / c(crossdist(start, end)))) * coords(start)$y
  })
