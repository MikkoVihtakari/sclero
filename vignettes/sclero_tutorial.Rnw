...
In addition several other features are planned, but they are not implemented yet. See Section \ref{sec:planned_features} for details.
...
\section{Features to be added} \label{sec:planned_features}

Currently the \sclero package includes features needed by the package author. There are a couple of features that I have half-ready code for and plan to add at some point in the future. These features are:

\begin{description}
\item[Measuring growth from ImageJ annotated photographs] Using the width of growth increments as environmental indicators is one of the most common approaches within schlerochronology. There are commercial (e.g. Image-Pro Plus) and open-source (ImageJ with ObjectJ plug-in) programs available for this task, but such measurements are relatively easy to implement. If implemented to R, the output could be directly linked with functions calculating growth indexes (particularly standardized growth index, SGI) sparing researchers from importing and exporting data.
\item[Calculating growth indexes] Calculating SGI and other growth indexes from the growth data would provide a complete feature with the point mentioned above.
\item[Measuring growth from photographs annotated with colored spots] Measuring growth patterns and doing spot alignment would also be possible without the step involving ImageJ by using any image handling program to annotate photographs with predefined colors. See  \href{http://stackoverflow.com/questions/19932853/how-to-extract-coordinates-of-colored-dots-from-a-jpeg-image}{this} Stack Overflow question for further information.
\end{description}
